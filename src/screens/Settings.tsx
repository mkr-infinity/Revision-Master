...
const Settings = () => {
  ...
  const [editCustomBaseUrl, setEditCustomBaseUrl] = useState(state.user.customBaseUrl || "");
  const [editCustomModel, setEditCustomModel] = useState(state.user.customModel || "");
  ...
  useEffect(() => {
    ...
    if (navState?.openModal === "aiSettings") {
      ...
      setEditCustomBaseUrl(state.user.customBaseUrl || "");
      setEditCustomModel(state.user.customModel || "");
      setCustomModels([]);
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[11px] text-tertiary-fg leading-tight capitalize">
                    {state.user.aiProvider === "custom"
                      ? `Custom · ${state.user.customModel || (state.user.customBaseUrl ? "No model" : "Not configured")}`
                      : `${state.user.aiProvider || "Gemini"} · ${(state.user.apiKeys?.[(state.user.aiProvider || "gemini") as AiProvider] || state.user.customApiKey) ? "Key configured" : "No key set"}`
                    }
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto no-scrollbar flex-1" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              {modalType === "aiSettings" && (
                <div className="space-y-5">
                  ...
                  {/* ── Custom Provider ── */}
                  {(() => {
                    const isActive = editProvider === "custom";
                    const isOpen = showKeyFor === "custom";
                    const hasToken = !!(editKeys.custom || "").trim();
                    const hasUrl = !!editCustomBaseUrl.trim();
                    return (
                      <div
                        className={`rounded-2xl border-2 transition-all ${
                          isActive
                        ...
                        <div className="flex items-center gap-3 p-3">
                          <button
                            ...
                            onClick={() => {
                              ...
                              setShowKeyFor(isOpen ? null : "custom");
                            }}
                            className="relative shrink-0 size-10 rounded-full flex items-center justify-center transition-transform active:scale-90"
                            style={{
                              background: "#6366f122",
                              boxShadow: (hasUrl || hasToken)
                                ? "0 0 0 2px #6366f1, 0 0 14px #6366f180"
                                : "0 0 0 1px #6366f155",
                            }}
                            aria-label="Toggle Custom Provider settings"
                          >
                            <span className="size-3.5 rounded-full" style={{ background: "#6366f1" }} />
                            {(hasUrl || editCustomModel) && (
                              <span className="absolute -bottom-0.5 -right-0.5 size-4 rounded-full bg-[var(--color-mint)] text-black flex items-center justify-center border-2 border-elevated">
                                <Check size={9} strokeWidth={3} />
                              </span>
                            )}
                          ...
                          <button
                            ...
                            onClick={() => {
                              setEditProvider("custom");
                              setShowKeyFor("custom");
                            }}
                            className="flex-1 text-left min-w-0"
                          >
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-sm truncate">Custom Provider</span>
                              {isActive && (
                                <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-[var(--color-primary)] text-white">
                                  Active
                                </span>
                            ...
                            <p className="text-[11px] text-tertiary-fg leading-snug truncate">
                              {editCustomModel
                                ? editCustomModel
                                : editCustomBaseUrl || "Together AI, Groq, Fireworks & more"}
                            </p>
                          </button>

                          <button
                            type="button"
                            onClick={() => setShowKeyFor(isOpen ? null : "custom")}
                            className="shrink-0 size-8 rounded-lg flex items-center justify-center text-tertiary-fg hover:bg-surface-2"
                            aria-label={isOpen ? "Hide custom settings" : "Show custom settings"}
                          >
                            {isOpen ? <EyeOff size={16} /> : <Eye size={16} />}
                          </button>
                        </div>
                        ...
                        <AnimatePresence initial={false}>
                          {isOpen && (
                            <motion.div
                              ...
                              className="overflow-hidden"
                            >
                              <div className="px-3 pb-3 pt-1 space-y-3 border-t border-subtle">
                                <p className="text-[11px] text-tertiary-fg leading-snug pt-1">
                                  Works with any hosted or self-hosted OpenAI-compatible API — Together AI, Groq, Fireworks, Mistral, OpenRouter, DeepSeek, and more.
                                </p>
                                <div>
                                  <label className="text-[10px] font-bold text-tertiary-fg uppercase tracking-widest block mb-1.5">
                                    Base URL
                                  </label>
                                  <input
                                    type="url"
                                    ...
                                    onChange={e => {
                                      setEditCustomBaseUrl(e.target.value);
                                      setCustomModels([]);
                                      setFetchModelsError(null);
                                    }}
                                ...
                                <div>
                                  ...
                                  <input
                                    type="text"
                                    value={editKeys.custom || ""}
                                    onChange={e => setEditKeys(prev => ({ ...prev, custom: e.target.value }))}
                                ...
                                <button
                                  type="button"
                                  disabled={!editCustomBaseUrl.trim() || fetchingModels}
                                  onClick={async () => {
                                    setFetchingModels(true);
                                    setFetchModelsError(null);
                                    setCustomModels([]);
                                    try {
                                      const base = editCustomBaseUrl.replace(/\/$/, "");
                                      const tok = (editKeys.custom || "").trim();
                                      ...
                                      try {
                                        const data = await tryFetch(`${base}/v1/models`);
                                      ...
                                      if (ids.length === 0) {
                                        try {
                                          const data = await tryFetch(`${base}/api/tags`);
                                          ids = (data.models || [])
                                            .map((m: any) => m.name || m.id).filter(Boolean);
                                        } catch (e: any) { lastErr = e.message; }
                                      }
                                      ...
                                      if (ids.length === 0) {
                                        try {
                                          const data = await tryFetch(`${base}/api/models`);
                                          ids = (data.data || data.models || [])
                                            .map((m: any) => m.id || m.name).filter(Boolean);
                                        } catch (e: any) { lastErr = e.message; }
                                      }
                                      ...
                                      if (ids.length === 0) {
                                        throw new Error(`Could not fetch models (${lastErr}). Check the base URL and ensure the server is reachable.`);
                                      }
                                      setCustomModels(ids);
                                      if (!editCustomModel || !ids.includes(editCustomModel)) {
                                        setEditCustomModel(ids[0]);
                                      }
                                    } catch (e: any) {
                                  ...
                                  className="w-full py-2.5 rounded-xl text-sm font-bold bg-indigo-500/15 text-indigo-400 hover:bg-indigo-500/25 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                                >
                                  <RefreshCw size={14} className={fetchingModels ? "animate-spin" : ""} />
                                  {fetchingModels ? "Fetching models…" : "Fetch Available Models"}
                                </button>
                                {fetchModelsError && (
                                  <p className="text-[11px] text-rose-500 font-medium">{fetchModelsError}</p>
                                )}
                                {customModels.length > 0 && (
                                  <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                      <label className="text-[10px] font-bold text-tertiary-fg uppercase tracking-widest">
                                        Select Model
                                      </label>
                                      <span className="text-[10px] text-tertiary-fg/60 font-medium">
                                        {customModels.filter(m => m.toLowerCase().includes(modelSearch.toLowerCase())).length} of {customModels.length}
                                      </span>
                                    </div>
                                    ...
                                    <div className="relative">
                                      <input
                                        ...
                                        className="w-full bg-app border border-subtle rounded-xl py-2 pl-8 pr-3 text-sm focus:ring-2 focus:ring-[#6366f1]/40 outline-none text-primary-fg placeholder:text-tertiary-fg"
                                      />
                                      <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-tertiary-fg pointer-events-none" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
                                      {modelSearch && (
                                        <button
                                          type="button"
                                          onClick={() => setModelSearch("")}
                                          className="absolute right-2.5 top-1/2 -translate-y-1/2 size-4 flex items-center justify-center rounded-full bg-tertiary-fg/20 text-tertiary-fg hover:bg-tertiary-fg/30"
                                        >
                                    ...
                                    {/* Model list */}
                                    <div className="rounded-2xl border border-subtle overflow-hidden bg-app max-h-48 overflow-y-auto no-scrollbar">
                                      {customModels
                                        .filter(m => m.toLowerCase().includes(modelSearch.toLowerCase()))
                                        .length === 0 ? (
                                        <div className="py-6 text-center">
                                          <p className="text-sm font-semibold text-tertiary-fg">No models match</p>
                                          <p className="text-[11px] text-tertiary-fg/60 mt-0.5">Try a different search term</p>
                                        </div>
                                      ) : (
                                        customModels
                                          .filter(m => m.toLowerCase().includes(modelSearch.toLowerCase()))
                                          .map((m, i, arr) => {
                                            const isSelected = editCustomModel === m;
                                            const parts = m.split("/");
                                            const modelLabel = parts[parts.length - 1];
                                            const modelOrg = parts.length > 1 ? parts[0] : null;
                                            return (
                                              <button
                                                key={m}
                                                type="button"
                                                onClick={() => setEditCustomModel(m)}
                                              ...
                                              >
                                                <div
                                                  className="shrink-0 size-7 rounded-lg flex items-center justify-center text-[10px] font-black"
                                                  style={{
                                                    background: isSelected ? "#6366f133" : "var(--bg-surface-2)",
                                                    color: isSelected ? "#6366f1" : "var(--text-tertiary)",
                                                  }}
                                                >
                                                  {modelLabel.slice(0, 2).toUpperCase()}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                  <p className={`text-sm font-semibold truncate leading-tight ${isSelected ? "text-indigo-400" : "text-primary-fg"}`}>
                                                    {modelLabel}
                                                  </p>
                                                  {modelOrg && (
                                                    <p className="text-[10px] text-tertiary-fg truncate leading-tight mt-0.5">{modelOrg}</p>
                                                  )}
                                                </div>
                                                {isSelected && (
                                                  <span className="shrink-0 size-5 rounded-full bg-indigo-500 flex items-center justify-center">
                                ...
                                {editCustomModel && customModels.length === 0 && (
                                  <div className="flex items-center gap-2.5 rounded-xl bg-indigo-500/10 border border-indigo-500/20 px-3 py-2.5">
                                    <span className="size-6 rounded-lg bg-indigo-500/20 flex items-center justify-center shrink-0">
                                      <Check size={12} className="text-indigo-400" strokeWidth={2.5} />
                                    </span>
                                    <div className="min-w-0">
                                      <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest leading-none mb-0.5">Active Model</p>
                                      <p className="text-sm font-mono font-semibold text-primary-fg truncate">{editCustomModel}</p>
                                    </div>
                                    <button
                                      type="button"
                                      onClick={() => setEditCustomModel("")}
                                      className="shrink-0 text-[10px] font-bold text-rose-400 hover:text-rose-300 ml-auto"
                                    >
                                      Clear
                                    </button>
                                  </div>
                    ...
                    <button 
                      onClick={() => {
                        ...
                        updateUser({
                          ...
                          customBaseUrl: editCustomBaseUrl.trim(),
                          customModel: editCustomModel.trim(),
                        });
                        setModalType("none");
