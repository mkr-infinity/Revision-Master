      ...
      <header className="p-6 pt-8 flex items-center justify-between sticky top-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md z-50">
        <div className="flex items-center gap-4">
          <div className="relative group cursor-pointer" onClick={() => {
            ...
            <div className="size-14 rounded-full border-2 border-primary p-0.5 relative overflow-hidden">
              <div
            ...
            <div className="absolute -bottom-1 -right-1 bg-primary text-[10px] text-white font-bold px-1.5 py-0.5 rounded-full border-2 border-background-dark">
              LV {state.user.level}
            </div>
          </div>
          <div>
      ...
      <main className="px-6 space-y-6">
        ...
        <section>
          <div className="flex justify-between items-end mb-3">
            <h3 className="text-lg font-bold">Current Goal</h3>
            <span
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-5 border-l-4 border-l-primary space-y-4">
                ...
                {state.user.examDate && (
                  <div className="flex items-center gap-3 mt-1.5">
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300 flex items-center gap-1.5">
                      ...
                      {new Date(state.user.examDate).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                    {daysLeft !== null && (
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${daysLeft > 30 ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : daysLeft > 7 ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : 'bg-red-500/20 text-red-600 dark:text-red-400'}`}>
            ...
            <div className="bg-white/50 dark:bg-black/20 rounded-lg p-3 border border-primary/10 relative">
              <Quote className="absolute top-2 left-2 text-primary/20" size={24} />
              <p className="text-sm italic text-slate-700 dark:text-slate-300 relative z-10 pl-6 pr-2 py-1">
                "{displayQuote}"
              </p>
            </div>
              ...
              {recentCards.map((card) => {
                ...
                return (
                  <div 
                    ...
                    className={`min-w-[240px] max-w-[240px] snap-center ${themeClass} border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm cursor-pointer hover:border-primary/50 transition-colors flex flex-col`}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <div className={`size-6 rounded flex items-center justify-center ${card.type === 'flashcard' ? 'bg-primary/10 text-primary' : 'bg-teal-500/10 text-teal-500'}`}>
                        {card.type === 'flashcard' ? <Layers size={12} /> : <FunctionSquare size={12} />}
                      </div>
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider truncate">
                        {card.deckName}
                      </span>
                    </div>
                    <p className="font-bold text-sm line-clamp-2 mb-2">{card.front}</p>
              ...
              {pinnedCards.map((card) => {
                ...
                return (
                  <div 
                    ...
                    className={`min-w-[240px] max-w-[240px] snap-center ${themeClass} border-2 border-primary/50 rounded-xl p-4 shadow-sm cursor-pointer hover:border-primary transition-colors flex flex-col`}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <div className={`size-6 rounded flex items-center justify-center ${card.type === 'flashcard' ? 'bg-primary/10 text-primary' : 'bg-teal-500/10 text-teal-500'}`}>
                        {card.type === 'flashcard' ? <Layers size={12} /> : <FunctionSquare size={12} />}
                ...
                <div className="absolute inset-0 backface-hidden rotate-y-180 bg-gradient-to-br from-primary/10 to-primary/5 border-2 border-primary/30 rounded-xl p-8 flex flex-col items-center justify-center text-center shadow-inner overflow-y-auto">
                  ...
                  {studyCard.image && (
                    <div className="mt-6 w-full max-h-60 rounded-xl overflow-hidden border-2 border-primary/20 shadow-md">
                      <img src={studyCard.image} alt="Card visual" className="w-full h-full object-contain bg-white/50 dark:bg-black/50" />
                    </div>
                  )}
                  {studyCard.notes && (
