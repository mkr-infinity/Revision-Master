...
const FormulaLibrary = () => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="backdrop-blur-xl bg-elevated/70 border border-white/[0.08] shadow-lg shadow-black/10 rounded-2xl px-4 py-3 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-xl leading-tight text-primary-fg">
                  Formula Library
                </h1>
      ...
      {createPortal(
        ...
        {studyCard && (
          ...
          >
            ...
            <div className="flex-1 min-h-0 p-6 flex flex-col items-center justify-center relative perspective-1000">
              <div 
                className={`w-full h-full min-h-0 transition-all duration-700 transform-style-preserve-3d cursor-pointer ${isFlipped ? 'rotate-y-180' : ''} shadow-2xl rounded-xl relative`}
                onClick={() => setIsFlipped(!isFlipped)}
              >
                {/* Front */}
                <div className="absolute inset-0 backface-hidden bg-elevated border-2 border-subtle rounded-xl p-8 flex flex-col items-center justify-between text-center shadow-inner overflow-y-auto">
                ...
                {/* Back */}
                <div className="absolute inset-0 backface-hidden rotate-y-180 bg-gradient-to-br from-primary/10 to-primary/5 border-2 border-primary/30 rounded-xl p-8 flex flex-col items-center justify-between text-center shadow-inner overflow-y-auto">
                  ...
                  {studyCard.notes && (
                    <div className="mt-6 p-4 bg-surface-2 rounded-xl text-sm italic text-secondary-fg w-full border border-subtle shadow-sm">
                      {studyCard.notes}
                    </div>
                  )}
                </div>
              ...
              <div className="mt-6 flex justify-center">
                <button 
                  onClick={() => setIsFlipped(!isFlipped)}
                  className="text-xs text-secondary-fg font-bold flex items-center gap-2 bg-surface-2 hover:bg-surface-3 px-4 py-2 rounded-full transition-colors shadow-sm border border-subtle"
                >
            ...
            <div className="p-4 border-t border-subtle flex gap-3 bg-elevated">
              <motion.button 
                whileTap={{ scale: 0.95 }}
                onClick={() => setStudyCard(null)}
