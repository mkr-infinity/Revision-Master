...
const MockTests = () => {
  ...
  const { state, logActivity, addMockTest, deleteMockTest } = useAppContext();
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      let promptText = `Generate a multiple-choice mock test with exactly ${actualNumCards} questions. `;
      let parts: any[] = [];

      if (genSource === "topic") {
        promptText += `The topic is: ${genTopic}.`;
        parts = [{ text: promptText }];
      } else if (genSource === "youtube") {
        promptText += `Analyze the following YouTube video URL and generate questions based on its content: ${genYoutubeUrl}`;
        parts = [{ text: promptText }];
      } else if (genSource === "pdf") {
        promptText += `Analyze the attached PDF document and generate questions based on its content.`;
        parts = [
          { text: promptText },
      ...
      const response = await ai.models.generateContent({
        ...
        contents: { parts },
        config: {
          systemInstruction: "You are MKR Ai, an AI assistant for Revision Master. Generate high-quality multiple-choice questions. Ensure there are exactly 4 options per question, and the correctAnswerIndex is between 0 and 3.",
      ...
      if (jsonStr) {
        const generatedQs = JSON.parse(jsonStr);
      ...
      alert("Failed to generate mock test. Please check your API key and internet connection.");
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const handleNextQuestion = () => {
    ...
    if (currentIndex < currentTest.questions.length - 1) {
      ...
      logActivity({
        type: "completed",
        itemType: "mock_test",
        itemName: `${currentTest.title} (${newResults.filter(r => r.isCorrect).length}/${newResults.length})`,
      });
    }
  };
            ...
            {genSource === "pdf" && (
              <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-8 text-center bg-slate-50 dark:bg-slate-800/50">
                ...
                <label htmlFor="pdf-upload" className="cursor-pointer flex flex-col items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-slate-700 dark:text-slate-300">Click to upload PDF</p>
            ...
            {!isSubmitted ? (
              <>
                <button 
                  ...
                  className="flex-1 py-4 rounded-xl font-bold bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 transition-colors"
                >
                  Skip
                </button>
                <button 
              ...
              <div className="flex-1 w-full space-y-6">
                <div>
                  <div className="flex justify-between items-end mb-2">
                    <p className="text-sm font-bold text-slate-600 dark:text-slate-400">Accuracy Progress</p>
                ...
                <div>
                  <div className="flex justify-between items-end mb-2">
                    <p className="text-sm font-bold text-slate-600 dark:text-slate-400">Speed (Qs/min)</p>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2 mb-4">
                      {result.question.options.map((opt, oIdx) => {
                        let badge = null;
                        let optClass = "text-slate-600 dark:text-slate-400";
                    ...
                    {result.question.explanation && (
                      <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-100 dark:border-slate-800">
                        ...
                        <p className="text-sm text-slate-700 dark:text-slate-300">{result.question.explanation}</p>
                      </div>
                    )}
                  </div>
                </div>
    ...
    >
      <header className="sticky top-0 z-50 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md px-4 py-4 flex items-center justify-between border-b border-primary/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold leading-none">Mock Tests</h1>
          </div>
        </div>
        <button 
      ...
      <main className="flex-1 overflow-y-auto px-4 max-w-2xl mx-auto w-full py-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Select Deck</label>
            <select 
