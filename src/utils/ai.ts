...
export type AiProvider = "gemini" | "openai" | "grok" | "anthropic" | "custom";
...
export const resolveProviderAndKey = (
  ...
  let apiKey: string | undefined = keys[provider as keyof ApiKeys]?.trim() || undefined;
  ...
  // Built-in fallback only for Gemini — reads the key baked in at build time
  if (provider === "gemini" && !apiKey) {
    const builtInKey = import.meta.env.VITE_GEMINI_API_KEY?.trim();
    apiKey = builtInKey || undefined;
  }
  // Custom provider: auth token is optional (some local servers don't need one)
  if (provider === "custom") {
...
const ensureKey = (provider: AiProvider, apiKey: string | undefined) => {
  // Custom provider auth token is optional
...
export const generateText = async (
  ...
  try {
    ...
    if (provider === "custom") {
      const baseUrl = user.customBaseUrl?.replace(/\/$/, "");
      if (!baseUrl) {
        throw buildError("custom", "missing_key", "Custom provider base URL is not set. Configure it in Settings → AI.");
      }
      const model = user.customModel;
      if (!model) {
        throw buildError("custom", "missing_key", "No model selected for Custom provider. Fetch and pick a model in Settings → AI.");
      }
